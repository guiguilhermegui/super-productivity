...
# Create a subtask (parent may be nested up to the depth cap; the subtask
# inherits the parent's projectId and cannot have its own tags)
curl -X POST http://127.0.0.1:3876/tasks \
  -H "Content-Type: application/json" \
...
| `parentId`           | Create the task as a subtask of this task. The parent may already be a subtask, but the resulting hierarchy must stay within the four-level task depth limit. Returns 404 if parent missing, 400 if the depth cap would be exceeded. The new subtask inherits the parent's `projectId` and never has its own tags — supplying `projectId` or `tagIds` together with `parentId` returns 400 `UNSUPPORTED_FIELD`. |
